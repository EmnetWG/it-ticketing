...
const {
  data:{categorys}
} = await axios.get(`/api/v1/subCategory/name?list.subCategoryName=${categoryname}`, {
  headers: {
    Authorization: `Bearer ${token}`,
  }},)
//console.log(categorys)
...
const allCategorys = categoryFilter
.map((subCategory) => {
const {name,  category, id:subCategoryID} = subCategory
return `
...
} = await axios.get(`/api/v1/tickets/departmentPendingTickets`, {
  headers: {
    Authorization: `Bearer ${token}`,
  }},)
...
.map((ticket) => {
var {subject, department, location, category, subCategory, approval, status, createdAt, createdBy,approvedBy, assignedTo, createdAt, remark, id:ticketID,
...
const {
  data:{tickets, totalTickets},
} = await axios.get(`/api/v1/tickets/departmentAcceptedTickets`, {
  headers: {
    Authorization: `Bearer ${token}`,
  }},)
...
const {
    data:{tickets},
} = await axios.get(`/api/v1/tickets/departmentTickets`, {
    headers: {
      Authorization: `Bearer ${token}`,
    }},)
...
*/
return `<tr>
<td colspan="2" style="font-size:14px">${subject}</td>
<td data-id="${departmentTicket.departmentId}">${departmentTicket.department}</td>
<td>${location}</td>
