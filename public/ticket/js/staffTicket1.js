...
.map((ticket) => {
var {subject, department, location, category, subCategory, approval, status, createdBy,approvedBy, assignedTo, remark, createdAt, id:ticketID,
...
const subCategoryValue = subCategorySelectUpdateDOM.options[subCategorySelectUpdateDOM.selectedIndex].dataset.id
...
const {
    data:{ticket},
} = await axios.patch(`/api/v1/tickets/${id}/updateStatus`, (statusValue && remarkValue  ? {status:statusValue, 
 remark:remarkValue}: ((statusValue) ? {status:statusValue}
: {remark:remarkValue}))
 , {
    headers: {
      Authorization: `Bearer ${token}`,
...
const {
  ...
  {
      ...((statusValue || remarkValue || subCategoryValue) ? {
          status: statusValue,
          remark: remarkValue,
