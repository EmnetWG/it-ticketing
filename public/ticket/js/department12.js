...
} = await axios.get(`/api/v1/tickets/departmentPendingTickets?page=${page}`, {
  headers: {
    Authorization: `Bearer ${token}`,
  }},)
...
prevButton.style.visibility = (currentPage === 1) ? 'hidden' : 'visible';
nextButton.style.visibility = (currentPage === noOfPages || noOfPages === 0) ? 'hidden' : 'visible';
...
const {
  data:{tickets, totalTickets, numOfPages},
} = await axios.get(`/api/v1/tickets/departmentAcceptedTickets?page=${page}`, {
  headers: {
    Authorization: `Bearer ${token}`,
  }},)
...
const {
    data:{tickets, totalTickets, numOfPages},
} = await axios.get(`/api/v1/tickets/departmentTickets?page=${page}`, {
    headers: {
      Authorization: `Bearer ${token}`,
    }},)
