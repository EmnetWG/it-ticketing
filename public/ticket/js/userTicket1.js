...
//{
 // return e.categorys.list.categoryid==id
//})
//const categoryFilter = categorys.subcategorys.filter(item => item.category===id)
//console.log({categoryFilter})
const allCategorys = categorys1
.map((subCategory) => {
...
subCategorySelectUpdateDOM.insertAdjacentHTML('beforeend', allCategorys)
//
  } catch(error) {
...
const allCategorys = categorys
.map((subCategory) => {
const {name,  category, id:categoryID, list} = subCategory
return `
<option data-id="${categoryID}">${name}</option>
`
})
...
subCategorySelectDOM.addEventListener('change', async (event) => {
  const categoryid2 = event.target.options[event.target.selectedIndex].dataset.id
  //return categoryid
  const subCategoryname2 = event.target.value
...
subCategorySelectUpdateDOM.addEventListener('change', async (event) => {
  const categoryid3 = event.target.options[event.target.selectedIndex].dataset.id
  //return categoryid
...
const {
    data:{tickets},
} = await axios.get(`/api/v1/tickets/requestedTickets`, {
    headers: {
      Authorization: `Bearer ${token}`,
    }},)
...
const {
  ...
  location:locationValue, department:departmentValue, 
