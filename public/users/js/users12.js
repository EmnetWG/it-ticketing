...
const resetButtons = document.querySelectorAll('.reset-btn');
    resetButtons.forEach(button => {
      button.addEventListener('click', async (event) => {
        ...
        const confirmed = window.confirm('Are you sure you want to reset this user password?');
        if (confirmed) {

          try {
            const {data:{user}}=await axios.patch(`/api/v1/users/${userID}/resetPassword`, {newPassword}, {
              headers: {
                Authorization: `Bearer ${token}`,
              },
            });
            ...
            usersDOM.innerHTML = '<h5 class="empty-list">There was an error reseting the user password, please try later....</h5>';
          }
        }
